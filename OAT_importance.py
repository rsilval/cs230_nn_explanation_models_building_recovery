...
r_list_train  = []
r_list_test = []
for i in range(0,size_data[1]-2):

        input_data = data[:,:(size_data[1]-2)]
        input_data = np.delete(input_data, i, axis=1)
        output_data = data[:,-1]
...
# #
#         with open('model_fitted_'+str(i)+'_.pkl', 'wb') as f:
#                 pickle.dump(mlp, f)
...
with open('OAT/r_test.pkl', 'wb') as f:
        pickle.dump(r_list_test, f)

with open('OAT/r_train.pkl', 'wb') as f:
